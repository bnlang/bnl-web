...
5. **Standard library** — built-in modules (`io`, `web`, `request`, `template`, `sqlite`, `pg`, `mysql`, `mongo`, `crypto`, ...) live in C++ and register their functions with the interpreter at startup.
</I18nEnglish>
...
৫. **স্ট্যান্ডার্ড লাইব্রেরি** — বিল্ট-ইন মডিউল (`io`, `web`, `request`, `template`, `sqlite`, `pg`, `mysql`, `mongo`, `crypto`, ...) C++ এ লেখা, এবং স্টার্টআপে ইন্টারপ্রেটারের সাথে নিজেদের ফাংশন রেজিস্টার করে।
</I18nBangla>
...
- **C++20** — the interpreter, value model, and embedding API.
...
- **SQLite / libpq / libmariadb / mongo-c-driver** — embedded SQLite plus first-class clients for PostgreSQL, MySQL/MariaDB, and MongoDB.
- **zlib** — gzip/deflate for HTTP compression and general data processing.

This is the entire dependency set — no V8, no Node.js runtime, no JS layer.
</I18nEnglish>

<I18nBangla>
ভেতরে Bnlang কিছু পরিচিত C/C++ লাইব্রেরির উপর দাঁড়ানো:

- **C++20** — ইন্টারপ্রেটার, ভ্যালু মডেল ও এমবেডিং API।
...
পুরো ডিপেন্ডেন্সি সেট এটুকুই — কোনো V8, Node.js রানটাইম বা JS লেয়ার নেই।
</I18nBangla>
