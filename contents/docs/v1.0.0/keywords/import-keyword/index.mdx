...
The standard library modules (`io`, `path`, `web`, `request`, `async`, `template`, `crypto`, `json`, `regex`, `time`, `timers`, `math`, `random`, `log`, `dotenv`, `cli`, `test`, …) ship with the runtime — no installation needed. Database clients (`sqlite`, `pg`, `mongo`, `mysql`) ship as separate bpm packages.
...
স্ট্যান্ডার্ড লাইব্রেরি মডিউল (`io`, `path`, `web`, `request`, `async`, `template`, `crypto`, `json`, `regex`, `time`, `timers`, `math`, `random`, `log`, `dotenv`, `cli`, `test`, …) রানটাইমের সাথে শিপ — কোনো ইনস্টলেশন দরকার নেই। ডাটাবেস ক্লায়েন্ট (`sqlite`, `pg`, `mongo`, `mysql`) আলাদা bpm প্যাকেজ হিসেবে আসে।
