...
async function fetchBlogs(opts: {
  ...
  const { page, limit, q, category, signal } = opts;
  const qs = buildQuery({
    page,
    limit,
    keyword: q,
  ...
  if (!res.ok) {
    const msg = `Failed to load blogs: ${res.status}`;
    throw new Error(msg);
  }
  ...
  return (
    <section className="relative isolate border-b mb-5">
      ...
      <div className="mx-auto max-w-5xl py-12 sm:py-16 text-center">
        <h1 className="text-3xl font-semibold tracking-tight">Bnlang Blogs</h1>
        <p className="mt-2 text-muted-foreground">
          Read blogs about Bangla language, programming, and more.
        </p>
...
export default function BlogsPage({ locale }: Props) {
  ...
  React.useEffect(() => {
    ...
    setQInput(_q);
  }, [sp.page, sp.q, sp.category]);

  const [data, setData] = React.useState<BlogsResponse>({
    total: 0,
    page: 1,
    limit,
    records: [],
  ...
  React.useEffect(() => {
    ...
    fetchBlogs({
      ...
      .catch((err) => {
        const msg = err?.message || "Failed to load blogs";
        toast.error(msg);
      })
      .finally(() => setLoading(false));

    return () => ac.abort();
  }, [page, limit, qApplied, category]);

  const clearFilters = () => {
    setCategory("All");
    setQInput("");
    pushQuery({ page: 1, q: "", category: "All" });
  };

  const onManualSearch = () => {
    pushQuery({ page: 1, q: qInput, category });
    setPage(1);
  };
