...
function makeInstallCommand(
  ...
  if (release) {
    if (os === "windows") {
      return `iwr -useb https://bnlang.dev/install.ps1 | iex`;
    } else {
      return `curl -fsSL https://bnlang.dev/install.sh | bash`;
    }
  }
  return "# No installer script or artifact found for this selection.";
}

export default function DownloadPage({ locale, data }: Props) {
  const t = useT(locale);
  ...
  // Initialize version to data.latest (if present and known) or the first
  // available version. Computed eagerly — no useEffect needed since data is
  // guaranteed at render time from getStaticProps.
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  // Read /releases.json at build time — no runtime API call, no loading state.
  const fs = await import("node:fs/promises");
  const path = await import("node:path");
  const file = path.join(process.cwd(), "releases.json");
  const raw = await fs.readFile(file, "utf8");
  const data: ApiResponse = JSON.parse(raw);

  return { props: { locale, data } };
};
