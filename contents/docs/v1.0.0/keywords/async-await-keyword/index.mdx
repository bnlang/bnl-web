...
banglishTitle: async · await
banglishDescription: Bnlang e osomoloy function & opekkha — sequential vs parallel, Promise.all, error handling, for‑await‑of.
---

# <I18nEnglish>async · await</I18nEnglish>
# <I18nBangla>অসমলয় · অপেক্ষা</I18nBangla>
# <I18nBanglish>async · await</I18nBanglish>
...
<I18nBanglish>
**`osomoloy`** function ekta **Promise** ferot dey. Vitor e **`opekkha`** dile promise resolve na howa porjonto thame thake ebong value pawa jay.  
Error handle & cleanup er jonno `ceshta/dhorun/obosheshe` use korun.
</I18nBanglish>
...
```bnl
// 1) Sequential
osomoloy function dataAno() {
  ceshta {
    const a = opekkha fetch("/a");
    const b = opekkha fetch("/b"); // /a shesh na hole shuru hobe na
    const c = opekkha fetch("/c");
    ferot [opekkha a.text(), opekkha b.text(), opekkha c.text()];
  } dhorun (err) {
...
osomoloy function drutoAno() {
  ceshta {
    const [a, b, c] = opekkha Promise.all([
      fetch("/a"),
      fetch("/b"),
      fetch("/c"),
    ]);
    ferot opekkha Promise.all([a.text(), b.text(), c.text()]);
  } dhorun (err) {
...
// 3) for‑await‑of on async iterable
osomoloy function poro(stream) {
  ceshta {
    jonno opekkha (const chunk er stream) {   // for await...of
...
// 4) Top‑level await (ES module)
// const config = opekkha fetch("/cfg.json").then(r => r.json());
```
  </TabsContent>
...
<I18nBanglish>
- `opekkha` default e **serial** kore; parallel korte hole age promise start korun, pore `Promise.all` ba alada kore `opekkha`.  
- Reject handle korte `ceshta/dhorun` use korun; cleanup er jonno `obosheshe`.  
- Independent task hole loop er vitor ek‑ek kore `opekkha` na kore batch + `Promise.all`.  
- `for opekkha...er` (for‑await‑of) **async iterable** e kaj kore.  
- **Top‑level `opekkha`** sudhu ES module e valid.  
