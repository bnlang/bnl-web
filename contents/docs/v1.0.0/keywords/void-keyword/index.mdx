...
banglishTitle: void Operator
banglishDescription: Bnlang e faka (void) — expression execute kore, kintu undefined ferot dey. Fire‑and‑forget, IIFE, link tips.
---

# <I18nEnglish>void Operator</I18nEnglish>
# <I18nBangla>ফাঁকা অপারেটর</I18nBangla>
# <I18nBanglish>void Operator</I18nBanglish>
...
<I18nBanglish>
**`faka`** operator expression **execute** kore kintu **protibar `undefined`** ferot dey.  
Side‑effect thakle result **ignore** korte `faka expr` use korun.
</I18nBanglish>
...
```bnl
faka expression
faka (expression)   // parenthesis optional
```
</I18nBanglish>
...
```bnl
// 1) Discard the value, keep side‑effects
let r = faka (console.log("running..."), 99);
console.log(r);              // undefined

// 2) Fire‑and‑forget call
faka kichu_korun();           // runs, result ignored (undefined)

// 3) IIFE style
faka function () {
  dekhao("immediate start");
}();
...
// 5) Historical: `faka 0` always undefined
console.log(faka 0);         // undefined
```
  </TabsContent>
...
<I18nBanglish>
- `faka` execution **bondho kore na**; sudhu `undefined` dey — `ferot` er cheye onno.  
- Itihasik vabe `faka 0` mane shobshomoy `undefined`; ajkal `undefined` read‑only, tobuo eta procholito.  
- `href="javascript:void 0"` diye navigation thama jaye — kintu **na korai bhalo**; **button**/`event.preventDefault()` use korun.  
- Result **ignore** korar signal dite `faka` upokari (especially async start), kintu beshi use korben na.  
