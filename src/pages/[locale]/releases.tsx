...
import { ApiRelease, ApiResponse } from "@/types/releases.types";
...
export default function ReleasesPage({ locale, data }: Props) {
  const t = useT(locale);
                ...
                <CardContent className="space-y-4">
                  {r.files?.length > 0 && (
                    <div className="grid md:grid-cols-2 gap-3">
                      {r.files.map((a) => {
                        const url = `${data.baseUrl ?? ""}${r.version}/${a.name}`;
                        return (
                            <div
...
export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  ...
  // Read /releases.json at build time so the page ships fully prerendered —
  // no runtime fetch, no loading state, no waterfall on first paint.
  // The path is resolved relative to the project root (where next runs).
